...
until wget -q "http://localhost:8086/ping" 2> /dev/null; do
    ...
    influx -host=localhost -port=8086 -execute="CREATE DATABASE ${INFLUXDB_GRAFANA_DB_CUSTOM_1}"
    influx -host=localhost -port=8086 -execute="CREATE DATABASE ${INFLUXDB_GRAFANA_DB_CUSTOM_2}"
    influx -host=localhost -port=8086 -execute="CREATE DATABASE ${INFLUXDB_GRAFANA_DB_CUSTOM_3}"
